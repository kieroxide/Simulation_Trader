...
class Simulator:
    ...
    def simulate(self, start_date="2010-01-04", end_date=dt.now()):
        ...
        while (current_date.date() != end_date.date()):
            if current_date.weekday() < 5:
                ...
                match random_decider:
                    case 1: #Buys Stock
                        price = read_current_stock_price(current_date, 'AAPL')
                        if not price:
                            continue
                        quantity = int((self.portfolio.balance * 0.1) / price) # 10% of balance can be used to buy
                        self.portfolio.buy_stock('AAPL', price, quantity)
                    case 2: #Sells Stock whole stock
                        ...
                        if stock_info:
                            ...
                            if not price:
                                continue
                            self.portfolio.sell_stock('AAPL', price, stock_info['quantity'])
                        else:
                            current_date -= td(days=1)
                self.portfolio.evaluate(current_date, 'AAPL', True)
            current_date += td(days=1)
